...
function addKeyAndValueToAll(array, key, value) {
    // Your code here
    return array.map(fruit => {fruit[key] = value; return fruit});
}
//console.log(addKeyAndValueToAll(fruits, "inStock", true));
// returns array of 31 fruits, and each fruit object includes "inStock: true"
...
function updateKeyName(array, oldKey, newKey) {
    // Your code here
    return array.map(fruit => {fruit[newKey] = fruit[oldKey] ; delete fruit[oldKey]; return fruit});
...
function updateIdValues(array) {
    // Your code here
    return array.map(fruit => {if(fruit.id > 9) fruit.id = '0000' + fruit.id; else fruit.id = '00000' + fruit.id; return fruit.id});
}
...
function deleteKeysAndValues(array, keyToDelete) {
    ...
    return array.map(fruit => {
        if(Object.keys(fruit).includes(keyToDelete)) delete fruit[keyToDelete];
        else delete fruit.nutritions[keyToDelete];
        return fruit;
    })
}
console.log(deleteKeysAndValues(fruits, "sugar"));
